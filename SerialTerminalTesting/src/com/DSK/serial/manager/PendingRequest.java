 ...
 *
 * [MeterRowDto 를 큐에 직접 넣지 않는 이유]
 * 폴링 도중 사용자가 테이블 행을 편집해도 이미 전송된 패킷 바이트(packet)는
 * 방어 복사(Arrays.copyOf)로 격리되어 영향받지 않는다.
 * 다만 MeterRowDto 참조는 파싱 메타데이터 용도로 읽기 전용 사용을 원칙으로 한다.
 */
