...
public class ModbusConfigPanel extends JPanel {
	...
	//	TODO: 오토폴링 부분 리팩토링 후 반영
	// 	configPanel.getConnectBtn().setEnabled(false);
	//	configPanel.getConnectProtocolBtn().setEnabled(false);
	// -> stopAuto, sendSelected, sendSelectedAuto
	...
	private void initConfigComponent() {
		...
		leftPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), "통신 프로토콜 설정",
		...
		for (JComboBox<?> combo : new JComboBox[] { endianCombo, dataTypeCombo }) {
			...
			combo.setFont(new Font("D2Coding", Font.PLAIN, 11)); // 가독성 좋은 고정폭 폰트 유지
		}
	...
	private void toggleCommunicationSetting() {
		if (!controller.isConnected()) {
			JOptionPane.showMessageDialog(this.terminal, "포트 연결이 해제된 상태입니다.");
			return;
		}
		if (terminal.getIsAutoPolling()) {
			JOptionPane.showMessageDialog(this.terminal, "검침중에 통신 제어 설정은 변경 불가합니다.");
			return;
		}
	...
	private void toggleConnection() {
		...
		if (rdoSerial.isSelected()) {
			...
			if (portCombo.getSelectedItem() == null) {
				JOptionPane.showMessageDialog(this.terminal, "연결할 COM 포트가 존재하지 않습니다.");
				return;
			}
			...
			if (controller.connectSerial(portName, baudRate, dataBits, stopBits, parity)) {
				...
				JOptionPane.showMessageDialog(this.terminal, "포트 개방에 실패하였습니다.");
			}
		} else {
			// --- [TCP/IP 연결 구역] 🌟 아래와 같이 구현합니다. ---
			...
			// 컨트롤러에 새롭게 추가할 connectTcp 메서드를 호출합니다.
			if (controller.connectTcp(ip, tcpPort)) {
				log.info("Modbus TCP connected: {}:{}", ip, tcpPort);
				updateConnectionState(true);
			} else {
				JOptionPane.showMessageDialog(this, "TCP/IP 서버 연결에 실패하였습니다.");
			}
		}
	}
	...
	public JButton getConnectProtocolBtn() {
		return connectProtocolBtn;
	}
}