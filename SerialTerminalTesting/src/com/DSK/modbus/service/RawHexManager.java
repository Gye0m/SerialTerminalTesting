...
/**
 * Raw HEX 송수신 전용 매니저. 수동/자동 검침과는 완전히 별개의 "부가 기능"이다.
 * 통신이 연결되어 있을 때 임의의 HEX 바이트를 직접 보내보고 응답을 그대로
 * 화면에 띄워주는, 어디까지나 임시 점검용 도구다.
 */
public class RawHexManager {

	private static final Logger log = LoggerFactory.getLogger(RawHexManager.class);

	// ✅ [수정] ModbusTerminal(View) 직접 참조 → ModbusEventListener 콜백으로 전환.
	private final ModbusEventListener listener;
	private final SerialConnectionManager connectionManager;

	private OutputStream outputStream;

	// 송신 직전 "다음 응답은 이 Slave ID 의 것" 이라고 PacketReceiver 에 알려주기
	// 위한 참조. null 이면(연결 전 호출 등) 그냥 건너뛴다.
	...
	public RawHexManager(ModbusEventListener listener, SerialConnectionManager connectionManager) {
		this.listener = listener;
		this.connectionManager = connectionManager;
	}

	/**
	 * PacketReceiver 연결. setExpectedSlaveId() 로 송신 직전 목적지 Slave ID 를
	 * 알려주기 위해 필요하다. SerialConnectionManager.connect() 안에서 한 번
	 * 호출된다.
	 */
	...
	// SerialConnectionManager.connect() 가 성공해서 포트가 열린 직후 호출된다.
	// =========================================================================
	public void bind() {
		try {
	...
	public synchronized void sendRawHex(String hex, boolean autoCrc) throws Exception {
		...
		// 송신 직전, 패킷 첫 바이트(목적지 Slave ID)를 PacketReceiver 에 알려준다.
		...
		SwingUtilities.invokeLater(() -> listener.onTxRx("[" + time + "][TX] " + txHex + "\n"));
		...
		timeoutTask = scheduler.schedule(() -> {
			...
			SwingUtilities.invokeLater(() -> listener.onTxRx("[TIMEOUT] 응답 없음\n"));
	...
	public synchronized void onReceive(byte[] packet) {
		...
		SwingUtilities.invokeLater(() -> listener.onTxRx("[" + time + "][RX] " + hex + " (" + elapsed + "ms)\n"));

		SwingUtilities.invokeLater(() -> listener.onRawResponse(hex + "\n"));
